...
// --- Configuration ---
const API_BASE_URL = "https://counterfeit-detection-1.onrender.com"; // IMPORTANT: Use your server's actual IP address
...
export default function ManufacturerPortal({ user, onLogout }) {
  ...
  // Load data from localStorage on initial rende
  useEffect(() => {
    try {
  ...
  return (
    <>
      <style>{`
        ...
        * {
        ...
        .header {
            ...
            z-index: 1000; background: rgba(28, 28, 28, 0.8);
            backdrop-filter: blur(10px); padding: 15px 30px;
        ...
        /* Card styles */
        .card {
            background: rgba(43, 43, 43, 0.8); backdrop-filter: blur(5px);
            padding: 40px;
        ...
        .card-title {
            font-size: 1.5rem; font-weight: 600; color: #f1f1f1;
        ...
        .btn-delete {
            ...
            padding: 5px 10px; font-size: 0.8rem; margin-left: 15px;
        }
      ...
      <div className="container">
        ...
        {view === 'wizard' && <BatchWizard onFinish={finishBatchCreation} user={user} />}
      </div>
    </>
  );
}
  ...
  <header className="header">
    ...
    <div className="profile-section">
      <div className="profile-info">
        <span className="profile-name">{user?.companyName || user?.email?.split('@')[0] || 'Manufacturer'}</span>
        <span className="profile-id">ID: {user?.uid.substring(0, 8) || 'N/A'}</span>
      </div>
...
const BatchWizard = ({ onFinish, user }) => {
  ...
  const handleAddManualProduct = (e) => {
    ...
    const newProduct = {
      ...
      mfgName: user?.companyName || "Acme Pharma",
  ...
  const handleFinishBatch = async () => {
    ...
    try {
      ...
      const batchResponse = await fetch(`${API_BASE_URL}/add_batch`, {
        method: 'POST',
      ...
      for (const product of batchData.products) {
        const productResponse = await fetch(`${API_BASE_URL}/add_product`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      const watermarkResponse = await fetch(`${API_BASE_URL}/embed_robust_watermark`, {
        method: 'POST',
        body: formData,
      });
    ...
    <div className="card">
      ...
      <div className={`wizard-step ${step === 1 ? 'active' : ''}`}>
        ...
        <form onSubmit={handleStep1Submit}>
          ...
          <div className="form-group"><label htmlFor="manufId">Manufacturer ID</label><input type="text" id="manufId" defaultValue={user?.uid || ''} required /></div>
